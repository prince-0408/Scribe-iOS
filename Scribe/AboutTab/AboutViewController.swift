...
extension AboutViewController {
    override func tableView(_: UITableView, heightForRowAt indexPath: IndexPath)
        -> CGFloat {
        let section = dataSet[indexPath.section]
        let setting = section.section[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .github:
            openURLString(
                urlString: "https://github.com/scribe-org/Scribe-iOS", withEncoding: false
            )

        case .matrix:
            openURLString(
                urlString: "https://matrix.to/#/#scribe_community:matrix.org", withEncoding: true
            )
    ...
    private func openURLString(urlString: String, withEncoding: Bool) {
        if withEncoding {
            ...
            guard let encodedURLString = encodedString, let url = URL(string: encodedURLString)
            else {
                return
            }
    ...
    private func openScribeAppStore(alert _: UIAlertAction) {
        openURLString(
            urlString: "itms-apps: //itunes.apple.com/app/id1596613886", withEncoding: true
        )
    }
    ...
    private func showEmailUI() {
        if MFMailComposeViewController.canSendMail() {
            ...
            let alert = UIAlertController(
                title: NSLocalizedString(
                    "i18n.app.about.feedback.send_email",
...
extension AboutViewController: MFMailComposeViewControllerDelegate {
    func mailComposeController(
        _ controller: MFMailComposeViewController, didFinishWith _: MFMailComposeResult,
        error _: Error?
    ) {
...
extension AboutViewController {
    private func showTipCardView() {
        ...
        hostingController.view.frame = CGRect(
            x: 0, y: 0, width: view.bounds.width, height: -40
        )
