        ...
        ZStack {
            ...
            HStack {
                ...
                Text(infoText)
                    .font(
                        Font.system(
                            size: DeviceType.isPad
                                ? 22 * textSizeMultiplier : 17 * textSizeMultiplier,
                            weight: .medium
                        )
                    )
                Spacer()
...
struct InstallationTipCardView: View {
    @AppStorage("installationTipCardState", store: .standard) var installationTipCardState: Bool =
        true

    var body: some View {
        TipCardView(
            infoText: NSLocalizedString(
                "i18n.app.installation.app_hint_tooltip",
